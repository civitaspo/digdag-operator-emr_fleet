...
class HelloOperatorFactory(val templateEngine: TemplateEngine)
  ...
  class HelloOperator private[example] (context: OperatorContext)
      extends BaseOperator(context) {
    override def runTask: TaskResult = { //Config params = request.getConfig();
      val params = request.getConfig.mergeDefault(
